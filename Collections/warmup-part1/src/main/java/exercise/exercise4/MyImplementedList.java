...
public class MyImplementedList<E> implements Iterable{
    ...
    private void ensureCapa() {
        int newSize = elementData.length *  INCREASE_SIZE_FACTOR;
    ...
    public  Object[] reverse (Object[] elementData) {
        Object[] objs = new Object[size];;
        int j = 0;
        for (int i  = size - 1; i >= 0; i--)
            objs[j++] = elementData[i];
        return objs;
    }
    ...
    public int lastIndexOf(Object obj) {
        Object[] objs = reverse(elementData);
        int index = 0;
        for (Object o: objs) {
            if (o.equals(obj)) {
                return objs.length - 1 - index;
            }
            index++;
        }
        return -1;
