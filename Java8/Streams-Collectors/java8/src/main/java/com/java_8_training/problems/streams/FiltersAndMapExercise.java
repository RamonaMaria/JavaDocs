...
//                Collectors.partitioningBy(x -> x%2 == 0, Collectors.summingInt(Integer::intValue)));
       // evenNumbers = .stream().collect(Collectors.partitioningBy(x -> x % 2 == 0));
       // Arrays.stream(array).map(i -> i % 2 == 0 ? i : -i).sum();
    ...
    public static List<String> findPersonByName(String name){
        ...
        personName = input.filter(x -> x == "Jack").collect(toList());
    ...
    public static List<Integer> filterNull(){
        ...
        evenNumbers = input.filter(x -> x != null).collect(toList());
    ...
    public static List<Integer> doubleNumbers() {
        ...
        doubleNumbers = input.map(integer -> integer *  2).collect(toList());
    ...
    public static List<Integer> flatteningLists() {
        ...
        together = input.flatMap(l -> l.stream()).collect(toList());
    ...
    public static int product(){
        ...
        int product = 1;
        //TODO #8
        product = input.reduce(1, (p, x)->p * x);
    ...
    public static Optional<Integer> findSquareDivisibleBy3(){
        ...
        firstSquareDivisibleByThree = someNumbers.stream().filter(x -> Math.sqrt(x) == (int)Math.sqrt(x)).filter(x -> x % 3 == 0).findFirst();
    ...
    public static int sumStringLength(){
        ...
        int sum = 0;
        //TODO #16
//        sum = strings.stream().reduce( (acc, x) -> acc + x.length());
        sum = strings.stream().mapToInt(String::length).reduce(0, (acc, x) -> acc +x );
        return sum;
    }
    ...
    public static  int sumSalary(List<Person> personList){
        int sum =0;
        //TODO #17
        sum = personList.stream().mapToInt(person -> person.getSalary()).reduce(0, (acc, x) -> acc +x );
        return sum;
    }
